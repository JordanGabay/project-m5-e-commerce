  ...
  return (<>
    ...
    ? <><PageNav>
      ...
      <label htmlFor='numItems'>Items per page:</label>
        <select name='numItems' defaultValue='24' onChange={(ev) => handleItemsPerPageChange(ev)}>
    ...
    {search && <SearchInfo>
      ...
      <ClearButton onClick={() => history.push('/products')}>X</ClearButton>
      </SearchInfo>}</>
    : search && <SearchInfo>
    ...
    <PriceFilter>
      <form >
        Price range: <PriceInput value={lowPrice === 0 ? '' : lowPrice} type='number' onChange={(ev) => setLowPrice(ev.target.value)} /> to <PriceInput value={highPrice === 0 ? '' : highPrice} type='number' onChange={(ev) => setHighPrice(ev.target.value)} />
      </form>
    </PriceFilter>
